...
import { usePaginatedTripsQuery, useDeleteTripMutation } from '../hooks/useTrips';
import { useDestinationsQuery } from '../../destinations/hooks/useDestinations';
...
export default function TripsList() {
    ...
    const filters =
        selectedDestination && selectedDestination !== "all"
            ? { "filter[destination]": selectedDestination }
            : {};
    ...
    return (
        <PageContainer>
            <div className="flex flex-col min-h-[85vh]">
                <AdminBreadcrumb items={[{ label: 'Dashboard', to: '/admin/dashboard' }, { label: 'Trips' }]} />

                {/* Header + Search + Destination Filter */}
                <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 mb-6 mt-4">
                    <h1 className="text-2xl font-bold">Trips</h1>

                    <div className="flex gap-3 flex-wrap md:items-center">
                        <Input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 overflow-auto flex-1 pb-24">
                    {isLoading ? (
                        ...
                        data.data.map((trip) => (
                            ...
                            >
                                <Link to={`/admin/trips/${trip.id}`} className="group relative overflow-hidden rounded-t-xl">
                                    <img
                                    ...
                                    <div className="absolute bottom-2 left-2 bg-black/50 text-white px-2 py-1 rounded-md text-sm">
                                        {formatCurrency(trip.egyptianPrice)} | {trip.duration}
                                    </div>
                                </Link>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {trip.highlights.map((highlight) => (
                                                <span
                                                    key={highlight}
                                                    className="text-xs px-2 py-1 rounded-full bg-green-50 text-green-700 border border-green-100 font-medium"
                                                >
                                                    {highlight}
                                                </span>
                                            ))}
                        ...
                        <p className="col-span-full text-center py-8 text-muted-foreground">No trips found.</p>
                    )}
                </div>
                ...
                <div className="sticky bottom-0 left-0 bg-white dark:bg-gray-900 border-t border-border py-3 flex justify-between items-center mt-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            ...
                            onClick={() =>
                                setPage((p) => Math.min(p + 1, data?.paginationResult.numberOfPages || p))
                            }
                        >
                            Next
                        </Button>
